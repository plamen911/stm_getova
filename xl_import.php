		...
		if(1 < $data->sheets[0]['numRows']) {
			...
			for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
				...
				if($tab == 'charts') {
					...
					if(isset($row[4])) {
						$r['published_date'] = (!empty($row[4]) && false !== $ts = strtotime($row[4])) ? date('Y-m-d H:i:s', $ts) : '';
						if(empty($r['published_date'])) continue;
					} else continue;
					if(isset($row[5])) {
						$r['hospital_date_from'] = (!empty($row[5]) && false !== $ts = strtotime($row[5])) ? date('Y-m-d H:i:s', $ts) : '';
						if(empty($r['hospital_date_from'])) continue;
					} else continue;
					if(isset($row[6])) {
						$r['hospital_date_to'] = (!empty($row[6]) && false !== $ts = strtotime($row[6])) ? date('Y-m-d H:i:s', $ts) : '';
						if(empty($r['hospital_date_to'])) continue;
					} else continue;
					if(isset($row[7])) { $r['days_off'] = intval($row[7]); } else continue;
					if(isset($row[8])) { $r['mkb_id'] = mb_strtoupper($row[8]); } else continue;
