...
<p class=MsoNormal>ЕГН <?=((isset($f))?HTMLFormat($f['egn']).', ':'')?><?php if(isset($f) && (''!=$f['l.location_name']||''!=$f['address'])) { ?>постоянен адрес: <?=((isset($f['r.province_name'])&&$f['r.province_name']!='')?'област '.HTMLFormat($f['r.province_name']).', ':'')?><?=((isset($f['c.community_name'])&&$f['c.community_name']!='')?'община  '.HTMLFormat($f['c.community_name']).', ':'')?><?=((isset($f['l.location_type']))?(($f['l.location_type']=='1')?'гр.':'с.'):'')?> <?=((isset($f['l.location_name'])&&$f['l.location_name']!='')?HTMLFormat($f['l.location_name']).', ':'')?><?=((isset($f))?HTMLFormat($f['address']):'')?><?php } ?></p>

<?php if(isset($f) && ''!=$f['d.doctor_name']) { ?><p class=MsoNormal style='text-align:justify'>Личен лекар: <?=HTMLFormat($f['d.doctor_name'])?>,
...
<ol style='margin-top:0cm' start=1 type=1>
 <li class=MsoNormal style='mso-list:l0 level1 lfo1;tab-stops:list 36.0pt'><b>Настоящ
     – </b><span style='mso-bidi-font-weight:bold'><?=mb_strtoupper(HTMLFormat($f['i.position_name']), 'utf-8')?> – от
...
<p class=MsoNormal>1.1. Длъжност: <b style='mso-bidi-font-weight:normal'><?=$dbInst->my_mb_ucfirst(HTMLFormat($f['i.position_name']))?><o:p></o:p></b></p>

<p class=MsoNormal>1.2. Работно място: <b style='mso-bidi-font-weight:normal'><?=$dbInst->my_mb_ucfirst(HTMLFormat($f['p.wplace_name']))?></b></p>
...
<?php $i = 1; if($f['i.position_workcond'] != '') { ?>
<p class=MsoNormal>1.3.1.<?=$i++?>. <?=HTMLFormat($f['i.position_workcond'])?></p>
<?php } if($f['i.position_workcond'] != '') { ?>
<p class=MsoNormal>1.3.1.<?=$i++?>. <?=HTMLFormat($f['p.wplace_workcond'])?></p>
<?php } ?>
...
<table class=MsoNormalTable border=0 cellspacing=0 cellpadding=0 width="100%"
 ...
 <tr style='mso-yfti-irow:1;mso-yfti-lastrow:yes'>
  ...
  switch ($line['stm_conclusion']) {
  	case '1':
  		echo '<p class=MsoNormal>Може да изпълнява посочената длъжност/професия '.HTMLFormat($f['i.position_name']).' в '.HTMLFormat($firm['name']).'</p>';
  		break;
  	case '2':
  		echo '<p class=MsoNormal>Може да изпълнява посочената длъжност/професия '.HTMLFormat($f['i.position_name']).' в '.HTMLFormat($firm['name']).' при следните условия:</p>';
  		echo '<p class=MsoNormal>'.HTMLFormat($line['stm_conditions']).'</p>';
  		break;
  	case '3':
  		echo '<p class=MsoNormal>Не може да изпълнява посочената длъжност/професия '.HTMLFormat($f['i.position_name']).' в '.HTMLFormat($firm['name']).'</p>';
  		break;
  	default:
  		break;
  }
