<?php
// Test: http://localhost/stm2008/stm_getova/w_rtf_worker_card.php?checkup_id=2942
...
$flds = array('smoking' => 'Тютюнопушене', 'drinking' => 'Алкохол', 'fats' => 'Нерационално хранене', 'diet' => 'Диета', 'home_stress' => 'Стрес в дома', 'work_stress' => 'Стрес в работата', 'social_stress' => 'Социален стрес', 'video_display' => 'Видеодисплей повече от 1/2 от раб. време', 'hours_activity' => 'Физическа активност часа /');
...
if($rows) {
	foreach ($rows as $row) {
		if(empty($row['doctor_desc'])) continue;
		$sect->writeText($dbInst->my_mb_ucfirst(HTMLFormat($row['doctor_pos_name'])).': '.HTMLFormat($row['doctor_desc']), $times12, $alignLeft);
	}
}
